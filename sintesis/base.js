...
(function () {
    ...
    render: function () {
      ...
      if (!entradas.length) {
        ...
        list.innerHTML = entradas.map(function (e) {
          ...
          var linea = p.aPedido ? 'A PEDIDO · ' + esc(p.plazo || '') + (e.tl !== 'U' ? ' · TALLE ' + esc(e.tl) + (p.mm && p.mm[e.tl] ? ' · ' + esc(p.mm[e.tl]) + ' MM' : '') : '') : unica ? 'RESERVADA — ES ÚNICA' + (e.tl !== 'U' ? ' · ' + varLabel(p) + ' ' + esc(e.tl) : '') : (esUltima(p) ? 'RESERVADA — QUEDA UNA' : 'EN TU CARRITO') + ' · ' + varLabel(p) + ' ' + esc(varTxt(p, e.tl));
  ...
  var LEGAL = {
    ...
    'talles-anillos': { t: 'Guía de talles · anillos', ja: '寸法', p: ['Los anillos de <b>Saga Jwls</b> se hacen a pedido: <b>cualquier diseño se adapta a cualquier talle</b>. La numeración es <b>USA</b>.', '<b>Cómo sacar tu talle:</b> apoyá sobre una regla un anillo que uses en ese dedo y medí el <b>diámetro interno</b>, de borde interno a borde interno. Buscá esa medida en la tabla y ese es tu talle. Si no tenés un anillo, medí el contorno del dedo con un hilo y usá la columna «circunferencia».'], guia: true },
  ...
  function legal(tema) {
    ...
    if (L.guia && w.CATALOGO && w.CATALOGO.saga) h += '<table class="pm-guia"><thead><tr><th scope="col">TALLE (USA)</th><th scope="col">DIÁMETRO INTERNO</th><th scope="col">CIRCUNFERENCIA</th></tr></thead><tbody>' +
      w.CATALOGO.saga.guia.map(function (g) { return '<tr><th scope="row">' + esc(g[0]) + '</th><td>' + esc(g[1]) + ' mm</td><td>' + esc(g[2]) + ' mm</td></tr>'; }).join('') + '</tbody></table>';
